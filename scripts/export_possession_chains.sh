...
mkdir -p "$(dirname "$OUT")"
bash "$ROOT/scripts/prune_local_cargo_artifacts.sh" "$TARGET" >&2
CARGO_INCREMENTAL="${CARGO_INCREMENTAL:-0}" CARGO_TARGET_DIR="$TARGET" nice -n 12 cargo build --release --bin soccer_export_possession_chains >&2
