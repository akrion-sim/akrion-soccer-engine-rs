...
fn train_ckpt(out_prefix: &str, games: usize, minutes: f64, seed_base: u32, ckpt_every: usize) {
    ...
    neural.target_popart_enabled = env_bool("SOCCER_NEURAL_TARGET_POPART", neural.target_popart_enabled);
    ...
    println!("[train-ckpt] done {games} games ({:.0}s)", started.elapsed().as_secs_f64());
}
...
fn play_fixture(
    ...
    let (home_id, away_id) = if cand_home { (0usize, 1usize) } else { (1usize, 0usize) };
    ...
    let (home, away) = if cand_home { (cand, base) } else { (base, cand) };
...
fn paired_stat(diffs: &[f64]) -> Stat {
    let n = diffs.len();
    if n == 0 {
        return Stat { mean: 0.0, lb95: 0.0, lb9999: 0.0, n };
    }
    let nf = n as f64;
...
fn eval(cand_spec: &str, base_spec: &str, games: usize, minutes: f64, holdout: u32) {
    ...
    let mut jsonl = jsonl_path.as_ref().map(|p| {
        use std::io::Write;
        let f = std::fs::OpenOptions::new().create(true).append(true).open(p).expect("open jsonl");
    ...
    for g in 0..games {
        ...
        acc!(c, cm, goals, shots, sot, att, comp, fwd, back, gain_yards, chains, shots_after_pass, assists, dribble_beats, route_one, interceptions);
        acc!(b, bm, goals, shots, sot, att, comp, fwd, back, gain_yards, chains, shots_after_pass, assists, dribble_beats, route_one, interceptions);
        ...
        if (g + 1) % 10 == 0 || g + 1 == games {
            eprintln!("[eval] {}/{games} played ({:.0}s)", g + 1, started.elapsed().as_secs_f64());
        }
    }
    if let Some(w) = jsonl.as_mut() {
        use std::io::Write;
    ...
    println!(
        ...
        c.att / nf, pct(c.comp, c.att), pct(c.fwd, c.comp), pct(c.back, c.comp),
        if c.comp > 0.0 { c.gain_yards / c.comp } else { 0.0 }, c.chains / nf
    );
    println!(
        "base: {:.1} att/g  {:.1}% comp  fwd {:.1}%  back {:.1}%  yards/pass {:.2}  chains {:.1}/g",
        b.att / nf, pct(b.comp, b.att), pct(b.fwd, b.comp), pct(b.back, b.comp),
        if b.comp > 0.0 { b.gain_yards / b.comp } else { 0.0 }, b.chains / nf
    );

    println!("\n----- guardrails (candidate vs baseline per game) -----");
    println!("goals:        {:.2} vs {:.2}   (GD {:+.2})", c.goals / nf, b.goals / nf, (c.goals - b.goals) / nf);
    println!("shots:        {:.2} vs {:.2}", c.shots / nf, b.shots / nf);
    println!("shots on tgt: {:.2} vs {:.2}", c.sot / nf, b.sot / nf);
    println!("shots-after-pass: {:.2} vs {:.2}", c.shots_after_pass / nf, b.shots_after_pass / nf);
    println!("assists:      {:.2} vs {:.2}", c.assists / nf, b.assists / nf);
    println!("dribble beats:{:.2} vs {:.2}", c.dribble_beats / nf, b.dribble_beats / nf);
    println!("route-one:    {:.2} vs {:.2}   (lower=better; long-ball regression guard)", c.route_one / nf, b.route_one / nf);
    println!("interceptions:{:.2} vs {:.2}", c.interceptions / nf, b.interceptions / nf);
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("train-ckpt") => {
            let prefix = args.get(2).expect("usage: train-ckpt <out_prefix> <games> <minutes> <seed_hex> <ckpt_every>");
        ...
        Some("eval") => {
            ...
            let base = args.get(3).expect("usage: eval <candidate|analytic> <baseline|analytic> ...");
