...
impl SoccerTeamGenome {
    ...
    fn sanitize(&mut self) {
        ...
        self.pass_willingness_under_pressure =
            self.pass_willingness_under_pressure.clamp(0.0, 1.0);
...
mod tests {
    ...
    fn genome_random_is_reproducible_and_distinct() {
        // Same seed ⇒ identical genome; different seeds ⇒ (almost surely) different.
        ...
        // Anchors stay on the 12×24 grid.
    ...
    fn genome_crossover_takes_genes_from_parents_and_survives_serde() {
        ...
        // Boolean genes must equal one parent's.
        assert!(child.use_scoop_pass == a.use_scoop_pass || child.use_scoop_pass == b.use_scoop_pass);
        assert!(child.sweeper_keeper == a.sweeper_keeper || child.sweeper_keeper == b.sweeper_keeper);
        // Genome round-trips through JSON (it is persisted as jsonb).
        ...
        assert_eq!(back.def_line_max_dist_from_ball, child.def_line_max_dist_from_ball);
    }
