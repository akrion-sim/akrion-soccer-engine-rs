...
/// to slip a firm forward ground ball to that breaking runner. **Default-ON in production**
/// (`DD_SOCCER_ENABLE_SLIP_BREAK_OFFSIDE=0/false/no/off` is the kill switch), read once.
/// **Default-OFF under test** (env-driven, re-read each call) so the option-scoring parity suite
/// stays byte-identical and individual tests can toggle it.
pub(crate) fn dd_soccer_enable_slip_break_offside() -> bool {
    ...
    #[cfg(not(test))]
    {
        ...
        *V.get_or_init(|| super::gate_default_on("DD_SOCCER_ENABLE_SLIP_BREAK_OFFSIDE"))
    }
}
...
/// his break, the passer and the runner share the credit. **Default-ON in production**
/// (`DD_SOCCER_ENABLE_SLIP_BREAK_OFFSIDE_REWARD=0/false/no/off` is the kill switch), read once.
/// **Default-OFF under test** so the reward-parity suite stays byte-identical.
pub(crate) fn dd_soccer_enable_slip_break_offside_reward() -> bool {
    ...
    {
        static V: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
        *V.get_or_init(|| super::gate_default_on("DD_SOCCER_ENABLE_SLIP_BREAK_OFFSIDE_REWARD"))
    }
}
...
mod tests {
    ...
    fn speed_advantage_rewards_a_faster_runner_and_a_line_stepping_up() {
        ...
        // A runner pulling away from a static line, and even more against a line stepping UP
        // (negative forward), reads as a stronger advantage.
        let vs_static = slip_break_speed_advantage(2.0, 0.0);
        let vs_stepping_up = slip_break_speed_advantage(2.0, -2.0);
        assert!(vs_static > 0.0 && vs_stepping_up > vs_static);
        // Saturates once the differential clears the reference.
