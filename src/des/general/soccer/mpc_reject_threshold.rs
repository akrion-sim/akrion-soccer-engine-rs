...
mod mpc_reject_threshold_tests {
    ...
    fn rwr_lowers_the_bar_when_low_p_commits_pay_off() {
        // Committing marginal (p = 0.2) moves in this context keeps working out
        // (reward above the batch baseline of the mixed corpus) ⇒ the learned bar for
        // this context should drop below the committed p, i.e. keep attempting them.
        let inputs = baseline_inputs();
        let good_p = 0.2;
        let samples: Vec<MpcRejectThresholdSample> = (0..48)
            .flat_map(|_| {
                [
                    // low-p commits that paid off
                    MpcRejectThresholdSample {
                        inputs: inputs.clone(),
                        committed_probability: good_p,
                        reward: 1.0,
                    },
                    // a higher-p commit that did poorly (baseline mixer)
                    MpcRejectThresholdSample {
                        inputs: inputs.clone(),
        ...
        let bar = head.predict(&inputs).expect("finite");
