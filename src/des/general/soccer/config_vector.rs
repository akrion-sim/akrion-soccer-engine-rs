...
mod tests {
    ...
    fn mirror_invariant_left_right() {
        ...
        for p in &mut mirrored.players {
            ...
            // Jerk is a kinematic VECTOR too — a left/right reflection negates its x
            // component, exactly like velocity/acceleration (the canonicaliser mirrors it).
    ...
    fn assigned_position_mirror_invariant() {
        ...
        for p in &mut mirrored.players {
            ...
            p.velocity = Vec2::new(-p.velocity.x, p.velocity.y);
            p.acceleration = Vec2::new(-p.acceleration.x, p.acceleration.y);
            // Jerk reflects with the board (negate x) like the other kinematic vectors.
            p.jerk = Vec2::new(-p.jerk.x, p.jerk.y);
