...
fn main() {
    ...
    // LEARNED-xT / goal-outcome reward surface (Codex's final reward-surface experiment). Replace the
    // analytic-shaped per-decision reward with the PURE goal outcome (+1 our goal / -1 conceded / 0),
    // identified as the large reward spikes (|r| >= PPO_GOAL_THRESHOLD). The policy then optimizes
    // toward WINNING rather than matching the analytic engine's positional preferences, and the critic
    // becomes the learned value surface (= learned xT). This changes the fixed point away from the
    // hand-coded analytic proxy — the one lever evidence hasn't ruled out. Gated PPO_GOAL_REWARD.
    if std::env::var("PPO_GOAL_REWARD").ok().as_deref() == Some("1") {
        let thr = env_f32("PPO_GOAL_THRESHOLD", 5.0);
        let (mut goals, mut concedes) = (0usize, 0usize);
        for t in trajs.iter_mut() {
            for d in t.iter_mut() {
                d.reward = if d.reward >= thr {
                    goals += 1;
                    1.0
                } else if d.reward <= -thr {
                    concedes += 1;
                    -1.0
                } else {
                    0.0
        ...
        println!("PPO_GOAL_REWARD on (thr={thr}): remapped to pure goal outcome — {goals} goal-for, {concedes} goal-against decisions");
    }
