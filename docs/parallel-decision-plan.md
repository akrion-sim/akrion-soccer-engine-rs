...
Perf math at 1/30: decision ~57 ms → ~18 ms (7 deciders) + serial floor → likely **~26–30 tps
single-threaded**. That may be enough on its own. Parallelism (below) is now **optional gravy**:
run the ~7 due-deciders on the pool if more margin is wanted — it composes (cadence picks *who*
decides; the pool decides them *in parallel*; both use `decide_player_on_snapshot`).
