...
### 1. Authoritative decision mode (engine patch)

Current local runs use `SOCCER_NEURAL_BLEND_MODE=authoritative`,
`SOCCER_NEURAL_BLEND_LAMBDA=1.0`, and `SOCCER_NEURAL_BLEND_WARMUP_STEPS=0`:

- `SoccerNeuralBlendMode::Authoritative` scores candidates as
  `λ·V_net(s,a) + 1e-6·Q_tab(s,a)`, so the neural value owns the legal-action ranking and the
  tabular value is only a deterministic tie-break floor. This replaces the old 0.5 nudge path.
- Warmth is persisted through `training_steps` and `average_loss` on the neural snapshot, so
  a reloaded net is not inert. The live `:5055` launcher keeps neural learning enabled for
  serving and refreshes local JSON sidecars, so the newest local-best snapshot can be installed
  without polling RDS.

### 2. Rebuilt the stack against the patched engine

- Trainer (`main_soccer_learning_run`) and the live server **dd-soccer-rs (`:5055`)** were
  both rebuilt against the patched engine and run in local-only JSON mode.
...
- Because authoritative mode is a match-level blend setting, both sides of an eval run
  authoritative unless the harness explicitly installs asymmetric configs. The "+105" is
  **vs the older reference net driving**, not vs the *pure* neural-off analytic engine. It
  cleanly proves **on-policy improvement** (+214 vs a fixed opponent); a definitive "beats
  the pure analytic engine" needs a per-team/asymmetric eval path so one side can be analytic
...
- Small-sample fixed-seed evals are noisy/deterministic — one 3-game run echoed an old
  ...
  trust.
- Getting a small local net to *decisively* exceed a mature analytic engine is a real,
  uncertain RL problem. The active local 64-hidden-unit net is still intentionally modest;
  what's proven so far is **direction**: with the neural authoritative and trained
  on-policy, learning now *visibly and measurably* changes play — which was the whole point.
