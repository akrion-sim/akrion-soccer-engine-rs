...
an **architecture** problem (the net was a bounded `Additive` nudge, lambda=0.5, structurally
forbidden from driving) and fixed it by making the neural **authoritative**:

- Live server defaults to `authoritative`, lambda=1.25 ([main_soccer_live.rs:476](../src/des/main_soccer_live.rs)).
- On-policy authoritative training climbed **delta-109 -> delta+105 (+214 Elo)** vs a *fixed
