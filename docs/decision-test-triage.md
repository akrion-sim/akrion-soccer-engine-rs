...
|---|---|---|
| ✅ `fatigue_accumulates_from_repeated_sprints_and_recovers_at_rest` | `high_after_sprints > 0.0` was false (fatigue stayed `0`) | **FIXED (test fixture).** Root cause was *not* broken plumbing — the fatigue math is correct (`MovementGait::fatigue_delta` → `Sprint = +0.0030·(1.55 − 0.80·cardio)·dt` ≈ +0.00225/s at elite stamina, applied via `add_fatigue` at `world.rs:14367`). The test sprinted toward `(25, 220)`, which `move_player_towards` **clamps to the pitch** (`y ≤ 120`); the player arrived early and then `Stand`'d, and standing recovery (`-0.020/s`) wiped the ~0.1 accumulated fatigue back to 0. Fix: loop each runner back to its start once it has covered most of the pitch so it keeps genuinely *sprinting* for all 48 s. Now green. (Pre-existing even at the old `HEAD` 7fb6053 — movement-calibration drift, not from any recent merge.) |
