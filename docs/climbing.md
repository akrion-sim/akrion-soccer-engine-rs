...
wired into a component that structurally *can't move behaviour*. We added
`DD_SOCCER_NEURAL_AUTHORITATIVE_LAMBDA` (=8): the neural value **dominates** the decision,
the tabular collapses to a tie-break floor. Now the net's learning shows up as behaviour.
...
so a loaded net actually drives. (And the authoritative path bypasses the ramp entirely.)
...
3. **Grow network capacity if it plateaus** — 24 hidden units on a 610-dim input is small;
   64/128 is the first lever once on-policy training flattens.
